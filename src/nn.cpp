...
af::array af_nn(RcppArrayFire::typed_array<f32> train_feats,
  ...
  if (dts == "f16" && !af::isHalfAvailable(devicen)) {
    std::cerr << "Half not available for device " << devicen << std::endl;
    return EXIT_FAILURE;
  }
...
//' @export
 // [[Rcpp::export]]
 af::array af_nn_sparse(RcppArrayFire::typed_array<f32, AF_STORAGE_CSR>& train_feats,
                 RcppArrayFire::typed_array<f32, AF_STORAGE_CSR>& test_feats,
                 RcppArrayFire::typed_array<s32>& train_target,
                 RcppArrayFire::typed_array<s32>& test_target,
                 int num_classes,
                 std::vector<int> layers,
                 const RcppArrayFire::typed_array<f32, AF_STORAGE_CSR>& query_feats,
                 bool relu_activation = false,
                 std::string device = "GPU",
                 std::string dts = "f32",
                 float learning_rate = 2.0,    // learning rate / alpha
